...
struct control{
	float acceleration,allowed_speed,speed,del_theta,kx,ky,steer_angle;
};
struct curve
{
	float kx, ky;
...
void compute_states(std::vector<state>& states, state current_states, std::vector<control>& controller, float step_size, float &length){
	...
	{
		states[j].velocity = controller[j].speed; //init_velocity+ controller[j].acceleration*step_size;
	...
	for (int i = 0; i < controller.size()-1; ++i)
	{
   	...
   	{
		if(i>0){
			length = length + sqrt((states[i].x-states[i-1].x)*(states[i].x-states[i-1].x) + (states[i].y-states[i-1].y)*(states[i].y-states[i-1].y));
			//theta = theta + abs(tan((path[i].ky - path[i-1].ky)/path[i].kx - path[i-1].kx));
		}
	}
...
void compute_u2(float input_length,std::vector<control> &controller,std::vector<curve> &curve_pts,float step_size, state init_state, state terminal_state, float speed_factor,float curve_factor){
	...
	for (int i = 0; i < n; ++i)
   	{
		...
		if((curve_pts[i].kx - curve_pts[i-1].kx) != 0){
			controller[i].del_theta = fabs(atan((curve_pts[i].ky - curve_pts[i-1].ky)/(curve_pts[i].kx - curve_pts[i-1].kx)) - last_heading);
			last_heading = fabs(atan((curve_pts[i].ky - curve_pts[i-1].ky)/(curve_pts[i].kx - curve_pts[i-1].kx)));
		}
		else
			controller[i].del_theta = 0;
		avg_delta = avg_delta + controller[i].del_theta;
		controller[i].allowed_speed = max_speed*speed_factor/(controller[i].del_theta * curve_factor + 1);
		if(i>0){
			length = length + sqrt((curve_pts[i].kx-curve_pts[i-1].kx)*(curve_pts[i].kx- curve_pts[i-1].kx) + (curve_pts[i].ky-curve_pts[i-1].ky)*(curve_pts[i].ky-curve_pts[i-1].ky));
		}
		 //std::cout<<i <<" length "<< length<<" x "<< curve_pts[i].kx << " y "<<curve_pts[i].ky  <<  std::endl;
	...
	for (int i = 0; i < n; i=i+time_diff)
	{	
		if((i+time_diff < n))
			controller[i].acceleration = (controller[i+time_diff].allowed_speed - init_velocity)/time_diff;
		else
			controller[i].acceleration = (terminal_state.velocity - init_velocity)/time_diff;
		for (int j = i; j < i+time_diff; ++j)
		{
			controller[j].speed = init_velocity + controller[j].acceleration;
			init_velocity = controller[j].speed;
			//std::cout<<"j "<<j<<" speed "<< controller[j].speed<<std::endl;	 
		}
 ...
 //  	ros::Subscriber speed = node.subscribe("speed_signal", 1, speed_callback);
	...
	while(ros::ok()){
		...
		if (target_flag == 2)
		{
			...
			way_points[1].x = 10;way_points[1].y = 0;
			way_points[2].x = 20;way_points[2].y = 0;
			way_points[3].x = 30;way_points[3].y = 10;
			way_points[4].x = 40;way_points[4].y = 30;
			...
			target_error = sqrt(pow((way_points[2].x - states[states.size()-1].x),2)+ pow((way_points[2].y - states[states.size()-1].y),2));
			std::cout << " error " << end_pointerror<< std::endl;

			while(end_pointerror >2 || end_pointerror<-2 ){//for(int i=0; i<10 ; i++ ){
				//if(end_pointerror > 2){
		...
		std::cout<< " 1 "<<states.size()<<std::endl;
		...
		// 			std::cout<< " speed "<<states[i].velocity<< std::endl;
		// 			//std::cout.flush();
		// 	}
