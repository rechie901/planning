...
void visualization_waypoints(visualization_msgs::Marker::Ptr marker,float x, float y,float red, float green, float blue){
	//visualization_msgs::Marker marker_goal;
	marker->header.frame_id = "odom";
	marker->header.stamp = ros::Time();
...
int main(int argc, char **argv){
	...
	//ros::Publisher vis_pub3 = node.advertise<visualization_msgs::Marker>( "waypoints", 1 );
	ros::Publisher map_pub = node.advertise<nav_msgs::Path>("map_path", 1);
	...
	visualization_msgs::Marker::Ptr waypoints_marker(new visualization_msgs::Marker);
	...
	std::vector<waypoints> map_data;
	nav_msgs::Path map_pts;
	...
	if (data_file.is_open())
	  {
	    while ( getline (data_file,line) )
	    {
	    	...
	    	geometry_msgs::PoseStamped pos;
	     	temp.x = std::stof (line,&sz);
	     	pos.pose.position.x = temp.x/100000;
  			temp.y = std::stof (line.substr(sz));
  			pos.pose.position.y = temp.y/1000000;
  			map_data.push_back(temp);
  			map_pts.poses.push_back(pos);
  			
  			//std::cout<< " out " << pos.pose.position.x <<" "<<pos.pose.position.y << std::endl; 
	    }
	   data_file.close();
	  }
	...
	while(ros::ok()){
		...
		// map_pts.header.frame_id = "odom";
		// map_pts.header.stamp = ros::Time::now();
		// map_pts.header.stamp = ros::Time::now();
		// map_pub.publish(map_pts);
		// for(int i; i< map_data.size(); i++){
		// 	float temp_x = map_data[i].x - 721979; 
		// 	float temp_y = map_data[i].y - 3.16059e+06;
		// 	visualization_waypoints(waypoints_marker, temp_x, temp_y,0,1,0);
		// }
		//vis_pub3.publish(*waypoints_marker);
